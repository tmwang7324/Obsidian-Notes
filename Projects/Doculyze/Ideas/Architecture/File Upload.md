# Overview
I used a v4 Pre-signed URL-based approach to upload files onto Firebase Cloud Storage.

Admin SDK writes the signed url.




# Dataflow 
Validate file type and size at client -> Infer file type using extension in server + Create Presigned URL based on filename extension (*skill.MD* -> text/markdown) + Mint Firestore record with -> Upload file through Presigned URL to Firebase Cloud Storage using `PUT` request -> Verify file size in Storage:
