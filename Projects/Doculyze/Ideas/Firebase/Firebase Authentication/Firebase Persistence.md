...
| **firebase.auth.Auth.Persistence.SESSION** | 'session' | Indicates that the state will only persist iun the current session or tab, and will be cleared when the tab or window in whcih the user authenticated is closed. Applies only to web apps.                                                                                                      |               |
