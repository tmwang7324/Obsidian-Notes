---
title: "Cross-tenant scheduled reap (backstop for stale-pending)"
...
blocked_by: ["[[0003-stale-pending-reap]]"]
project: Doculyze
---

# 09 — Cross-tenant scheduled reap (backstop for stale-pending)

**Parent:** [[0001-SPEC-data-architecture]]

**Status:** deferred / learning exercise — *not* ready-for-agent.

**What to build:** A session-less background job that reaps abandoned `pending` uploads across **all** users, regardless of who is browsing — the one population no in-request trigger can reach (a user who abandons an upload and never returns).
...
- [ ] `reapAllStalePendingDocuments()` selects stale `pending` records across **all** users via a `collectionGroup` indexed query on status + age — never a bucket walk, never per-user iteration in app code.
