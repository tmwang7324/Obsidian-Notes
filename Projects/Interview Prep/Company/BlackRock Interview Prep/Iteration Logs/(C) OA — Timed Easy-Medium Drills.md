...
> **Resolved 2026-08-08 — closed with the pipeline.** The BlackRock loop went silent after
> the ~2026-07-11 OA deadline (see [[(C) Land BlackRock Aladdin Offer]], `status: stalled`).
> **The drill set below is the salvage** — the unchecked LC problems (Priorities A–E) are
> firm-agnostic and belong in shared prep; carry them to `Interview Prep/Leetcode/` rather
> than working them under a dead pipeline. The BlackRock-specific reported-OA questions
> stay here as an archived record.

**Done when (original):** I can take a cold Easy–Medium problem, parse its input, solve it,
and format output within a realistic time budget, twice in a row without a harness bug.
...
### Recommended Drill Set — moved out 2026-08-08

The Easy→Medium drill set that lived here was **firm-agnostic**, so it moved up to shared
prep when this pipeline stalled:

→ **[[(C) Shared OA Drills — Self-Parsed Easy-Medium]]** (`Interview Prep/Iteration Logs/`)

That file carries the unchecked problems (RPN 150, Evaluate Division 399, Kadane 53,
Climbing Stairs 70, House Robber 198, LCS 1143), cross-links the ones a Google backlog
item already owns, and records what was already finished. **Work them there.**

The BlackRock-specific reported-OA questions above stay here as an archived record of
what this firm actually asked.