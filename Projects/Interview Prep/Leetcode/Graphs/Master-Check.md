# 1. Write BFS and DFS from scratch
...
# 2. What mistakes did you make writing BFS and DFS, and what is the key invariant to avoid them?
...
# 3. Derive the time and space complexity of BFS and DFS
...
# 4. Write Union-Find from scratch. Explain the parent and rank invariants, derive the complexity, and explain the Ackermann function.
```python
def find(n1):
	res = n1
	while res != par[res]:
...
# 5. Which algorithm is best for each scenario?
**(a) Finding the shortest path from A to B:**
BFS due to the fact that the shortest path should be whenever B is found. Cheaper than DFS

**(b) Finding connected components:**
Union-Find because connected components are disjoint sets.

**(c) Detecting a cycle:**
DFS is the best algorithm to use because one can compare the current node's neighbors to the visited set. If one adds a visited node into the current node's adjacency list, then one will have created an edge leading to a cycle.