...
See `index.md` for the page catalog and `log.md` for recent activity. As of 2026-07-06: six active projects — **Doculyze** (full-stack dev / auth), **Obsidian Configs** (vault infrastructure / maintenance), **Golf** (deliberate practice across driving, irons, short game, and putting), **Google Interview Prep** (topic-by-topic DSA / system-design prep toward a New Grad SWE offer, targeting interview-readiness before the July 27, 2026 Barclays start), **NYC Neighborhood Commercial Analysis** (full-stack resume project — a Next.js/FastAPI platform that ranks 71 NYC neighborhoods for a prospective business via embedding relevance + a competitive-saturation score), and **BlackRock Interview Prep** (short-fuse, deadline-driven prep for the BlackRock/Aladdin online assessment + HireVue behavioral) — and 17 wiki pages. Each project lives in `Projects/<name>/` with its own `CLAUDE.md` + `COMMANDS.md`.
