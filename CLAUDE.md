...
See `index.md` for the page catalog and `log.md` for recent activity. As of 2026-07-16: eight active projects — **Barclays** (the day job — Technology Analyst, start **July 27, 2026**; onboarding, first 90 days, benefits/401(k), reviews, and long-term growth at the firm; strict rule: **never log Barclays-internal specifics** — this vault is a git repo), **Doculyze** (full-stack dev / auth), **Obsidian Configs** (vault infrastructure / maintenance), **Golf** (deliberate practice across driving, irons, short game, and putting), **Google Interview Prep** (topic-by-topic DSA / system-design prep toward a New Grad SWE offer, targeting interview-readiness before the July 27, 2026 Barclays start), **NYC Neighborhood Commercial Analysis** (full-stack resume project — a Next.js/FastAPI platform that ranks 71 NYC neighborhoods for a prospective business via embedding relevance + a competitive-saturation score), **BlackRock Interview Prep** (short-fuse, deadline-driven prep for the BlackRock/Aladdin online assessment + HireVue behavioral), and **Grad School** (deliberately thin hedge — protect the NYU professor's recommendation letter and decide PhD-vs-Master's-vs-neither; real program research deferred to September) — and 17 wiki pages. Each project lives in `Projects/<name>/` with its own `CLAUDE.md` + `COMMANDS.md`.
