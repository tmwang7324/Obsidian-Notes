...
└── Projects/
    ...
    └── Interview Prep/
        ├── Leetcode/ ← shared DSA / coding-pattern material across interview loops
        └── Company/<Company Interview Prep>/ ← company-specific prep subprojects
```
...
- **Parent projects** — a top-level project may organize subprojects when the work naturally shares infrastructure. `Projects/Interview Prep/` is the canonical example: shared DSA material lives in `Leetcode/`, while company-specific loops live in `Company/<Company Interview Prep>/` and keep their own `CLAUDE.md`, `COMMANDS.md`, `Goals/`, `Progress/`, and `Iteration Logs/`.
...
See `index.md` for the page catalog and `log.md` for recent activity. As of 2026-07-20: seven active top-level projects - **Barclays** (the day job - Technology Analyst, start **July 27, 2026**; onboarding, first 90 days, benefits/401(k), reviews, and long-term growth at the firm; strict rule: **never log Barclays-internal specifics** - this vault is a git repo), **Doculyze** (full-stack dev / auth), **Obsidian Configs** (vault infrastructure / maintenance), **Golf** (deliberate practice across driving, irons, short game, and putting), **Interview Prep** (parent recruiting-prep project with shared Leetcode material and company subprojects for Google and BlackRock), **NYC Neighborhood Commercial Analysis** (full-stack resume project - a Next.js/FastAPI platform that ranks 71 NYC neighborhoods for a prospective business via embedding relevance + a competitive-saturation score), and **Grad School** (deliberately thin hedge - protect the NYU professor's recommendation letter and decide PhD-vs-Master's-vs-neither; real program research deferred to September) - and 17 wiki pages. Each top-level project lives in `Projects/<name>/` with its own `CLAUDE.md` + `COMMANDS.md`; company-specific interview prep lives under `Projects/Interview Prep/Company/<name>/`.
