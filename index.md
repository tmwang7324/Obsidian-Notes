...
_Last updated: 2026-08-09 · 57 pages_
...
| [[(C) RAG Ingest and Query Data Architecture\|RAG Ingest and Query Data Architecture]] | **Spec** — mint-first lifecycle, pull-based ingest, two Python containers, Chroma tenant gateway; supersedes parts of the older Doculyze architecture pages (unreconciled) |
...
| Page | Summary |
|---|---|
| [[(C) Reranking\|Reranking]] | Query-time cross-encoder pass over retrieved candidates; why it *can't* run at index time (bi- vs cross-encoder), the recall/ordering split, k as the latency knob |
...
| [[(C) Token Verification and Revocation\|Token Verification and Revocation]] | Role vs storage vs transport; why the access/refresh split is about **revocation, not identity**; XSS↔CSRF tradeoff; why access checks are cheap (they check less); revocation window = access-token lifetime; Firebase *is* dual-token + `checkRevoked` |
